...
> 基于用户自主获取账单的个人财务核查 Agent。

用户上传本人从银行或支付平台导出的账单，系统形成可追溯账本，支持查询、分类修正、异常核查和结果导出。分析范围始终限于已导入的数据，并标明账户、币种和期间。
...
| 导出 | 导出选定期间的数据，金额和币种保持原值 |
| Agent | 把自然语言任务转换成受约束查询，返回确定性统计、规则与交易依据 |
