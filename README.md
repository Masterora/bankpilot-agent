...
系统只分析已导入的账单，不登录网银、不读取实时余额、不执行资金操作。月度报告支持生成、历史查看、更新提示和完整证据导出；周期扣款与预算保留入口，尚未交付。
...
[最终产品原型](docs/final-product.html)保留为产品方向参考，使用合成数据且不连接后端，其中周期扣款、预算等交互不代表已实现能力；当前交付范围以交付与验证文档为准。
