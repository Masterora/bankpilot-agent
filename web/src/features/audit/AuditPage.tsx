...
export function AuditPage({ copy, run, locale }: { copy: Messages; run: Run | null; locale: Locale }) {
  ...
  return (
    <section className="product-page audit-page">
      ...
      {!!history.length && <div className="import-table-wrap audit-history"><table className="import-table"><thead><tr><th>{english ? 'Time' : '运行时间'}</th><th>{english ? 'Task' : '核查任务'}</th><th>{english ? 'Status' : '状态'}</th><th>{english ? 'Details' : '详情'}</th></tr></thead><tbody>{history.map(item => <tr key={item.id} className={selectedId === item.id ? 'selected-row' : ''}><td className="time-cell">{formatTimestamp(item.created_at, locale)}</td><td>{item.message}</td><td><span className={`status status-${item.status.toLowerCase()}`}>{copy.statuses[item.status as Run['status']] ?? item.status}</span></td><td><button aria-pressed={selectedId === item.id} onClick={() => { if (selectedId === item.id) return; setSelected(null); setFailed(false); setLoading(true); setSelectedId(item.id) }}>{english ? 'View' : '查看'}</button></td></tr>)}</tbody></table></div>}
